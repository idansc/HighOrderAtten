...
function DataLoader:__init(opt)
    ...
    if opt.h5_img_file_train ~= nil then
        ...
        self.MC_ans_train = h5_file:read('/MC_ans_train'):all()
    ...
    if(self.eval and self.eval>0) then
...
function DataLoader:getBatch(opt)
    ...
    if split == 0 or split == 1 then
		...
		data.mc_answer = self.MC_ans_train:index(1, ques_idx)
    else
        data.images = self.img_batch:view(batch_size, 196, -1):contiguous()
		...
		data.mc_answer = self.MC_ans_test:index(1, ques_idx)           
		if(self.eval and self.eval>0) then
