local CompactTrilinearPooling, parent = torch.class('nn.CompactTrilinearPooling', 'nn.Module')
...
function CompactTrilinearPooling:__init(outputSize, path_name,homogeneous)
...
function CompactTrilinearPooling:sample()
   file_path = 'save/tcb_params_' .. self.path_name .. '.t7'
   if(file_exists(file_path)) then
       load_params = torch.load(file_path)
       self.h1:copy(load_params.h1)
       self.h2:copy(load_params.h2)
       self.h3:copy(load_params.h3)
       self.s1:copy(load_params.s1)
       self.s2:copy(load_params.s2)
       self.s3:copy(load_params.s3)
   else
       self.h1:uniform(0,self.outputSize):ceil()
       self.h2:uniform(0,self.outputSize):ceil()
       self.h3:uniform(0,self.outputSize):ceil()
       self.s1:uniform(0,2):floor():mul(2):add(-1)
       self.s2:uniform(0,2):floor():mul(2):add(-1)
       self.s3:uniform(0,2):floor():mul(2):add(-1)
       torch.save('save/tcb_params_' .. self.path_name .. '.t7', {h1=self.h1, h2=self.h2, h3=self.h3, s1=self.s1, s2=self.s2, s3=self.s3})
    end
end


function CompactTrilinearPooling:psi()
   self.y:zero()
   ...
   for i=1,3 do
	 ...
	 elseif i==2 then
       ...
       self.y[i]:indexAdd(2,self.h3,torch.cmul(self.s3:repeatTensor(batchSize,1),self.input[i]))
	 end
    end
   end
end

function CompactTrilinearPooling:conv(x,y,z)
   ...
   local function makeComplex(x,y)
      ...
      self.z_ = self.z_ or torch.CudaTensor()
      self.z_:resize(z:size(1),1,1,z:size(2),2):zero()
      self.z_[{{},{1},{1},{},{1}}]:copy(z)
   end
   makeComplex(x,y,z)
   ...
   self.fft_z = self.fft_z or torch.CudaTensor(batchSize,1,1,dim,2)
...
function CompactTrilinearPooling:updateOutput(input)
   ...
   self.output = self:conv(self.y[1], self.y[2], self.y[3])
   if self.debug then print('conv:', sys.toc(1)) end

   return self.output
end

function CompactTrilinearPooling:updateGradInput(input, gradOutput)
   ...
   for k=1,3 do
      ...
      self.tmp = self:conv(gradOutput, self.y[i],self.y[j])
      if k==1 then
      ...
      elseif k==2 then
        ...
        self.gradInput[k]:index(self.tmp, 2, self.h3)
        self.gradInput[k]:cmul(self.s3:repeatTensor(batchSize,1))
      end
   end
